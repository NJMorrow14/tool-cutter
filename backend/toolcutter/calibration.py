...
def rectify(img: np.ndarray, corners: Sequence[Sequence[float]], width_mm: float, height_mm: float,
    ...
    ppm = ppm_override or choose_px_per_mm(ordered, width_mm, height_mm, max_side=max_side)
    ...
    for arr in extra or []:
        ...
        if np.issubdtype(arr.dtype, np.floating) and not np.isfinite(arr).all():
            ...
            weight = cv2.warpPerspective(valid.astype(np.float32), H, (out_w, out_h), flags=cv2.INTER_LINEAR,
                                         borderMode=cv2.BORDER_CONSTANT, borderValue=0)
            total = cv2.warpPerspective(np.where(valid, arr, 0).astype(np.float32), H, (out_w, out_h), flags=cv2.INTER_LINEAR,
                                        borderMode=cv2.BORDER_CONSTANT, borderValue=0)
            value = np.full(weight.shape, np.nan, np.float32)
            np.divide(total, weight, out=value, where=weight >= .5)
            extras_out.append(value)
        else:
