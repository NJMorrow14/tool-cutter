...
def _load_points(data, ext: str, target_points: int = 3_000_000):
    """Return (points Nx3 float64, colors Nx3 uint8 or None). See _load_points_full."""
    pts, colors, _ = _load_points_full(data, ext, target_points)
    return pts, colors


def _load_points_full(data, ext: str, target_points: int = 3_000_000):
    """Return (points, colors or None, surface normals or None). `data` is bytes, or a path (str/Path) so
    that OBJ material and texture files next to it resolve. Normals come from mesh faces (oriented
    outward by every scanner/photogrammetry pipeline) and are None for pure point clouds."""
    import trimesh  # type: ignore

    ext = ext.lower().lstrip(".")
    if isinstance(data, (str, os.PathLike)):
        loaded = trimesh.load(str(data), force=None, process=False)
    else:
    ...
    pts_list, col_list, nrm_list, has_color, has_normals = [], [], [], True, True
    for m in meshes:
        if len(m.faces) == 0:
        ...
        samples, face_idx = trimesh.sample.sample_surface(m, n, seed=0)   # deterministic rasters
        pts_list.append(np.asarray(samples, dtype=np.float64))
        pts_list.append(np.asarray(m.vertices, dtype=np.float64))
        nrm_list.append(np.asarray(m.face_normals[face_idx], dtype=np.float64))
        nrm_list.append(np.asarray(m.vertex_normals, dtype=np.float64))
        colors = None
        try:
    ...
    normals = np.vstack(nrm_list) if has_normals and nrm_list else None
...
def rasterize_scan(data: bytes, ext: str, units: str = "auto", max_side: int = 2000,
                   min_mm_per_px: float = 0.4, pts_colors=None, normals: Optional[np.ndarray] = None,
                   flip_normal: bool = False) -> ScanRaster:
    ...
    # which side is "up"? Mesh face normals on the floor point at the camera that saw it: definitive.
    # (The point-count heuristic inside fit_plane_ransac is only a fallback for bare point clouds.)
    if normals is not None:
        agree = float(np.mean(normals[inl] @ n))
        if abs(agree) > 0.2 and agree < 0:
            n, d = -n, -d
    if flip_normal:
        n, d = -n, -d
