...
class Segmenter:
    ...
    def automatic_masks(self, image_bgr: np.ndarray, key: Optional[str] = None) -> list:
        """Discover objects in the displayed image without misregistered depth prompts.

        A small batch bounds accelerator memory; the image encoder runs once.
        The shared model is locked for the complete generation operation.
        """
        self.ensure_loaded()
        from segment_anything import SamAutomaticMaskGenerator
        with self._lock:
            if key is not None and self._automatic_cache is not None and self._automatic_cache[0] == key:
                return self._automatic_cache[1]
            def generate():
                generator = SamAutomaticMaskGenerator(
                    self._predictor.model, points_per_side=16, points_per_batch=8 if self.device in ("mps", "cuda") else 4,
                    pred_iou_thresh=0.88, stability_score_thresh=0.94,
                    min_mask_region_area=100)
                return generator.generate(cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB))
            try:
                masks = generate()
            except (RuntimeError, TypeError) as exc:
                if self.device != "mps":
                    raise
                log.warning("Automatic masks failed on Metal: %s", exc)
                # Retain a CPU fallback for unsupported accelerator operations.
                self._fallback_to_cpu()
                masks = generate()
            self._automatic_cache = (key, masks) if key is not None else None
            return masks
